...
fn parse_the_architectures(input_line: &str) -> Vec<String> {
...
fn parse_the_components(input_line: &str) -> Vec<String> {
    let index = input_line
        .find(':')
...
/// parses a codename
fn parse_a_codename(input_line: &str) -> String {
    let index = input_line
        .find(':')
        .expect("A : went missing in parse_a_codename");
    input_line[(index + 1)..].trim().to_string()
}

/// parse a suite tag
fn parse_a_suite(input_line: &str) -> String {
    let index = input_line
        .find(':')
...
/// parses a date tag
fn parse_a_date(input_line: &str) -> DateTime<Utc> {
